...
def recomendacion_juego(id_producto):
    ...
    juego_filtrado_vector = np.array([1 if genre in juego_filtrado['genres'].iloc[0] else 0 for genre in genero_juego])
    juegos_recomendados['similarity'] = juegos_recomendados['genres'].apply(lambda x: cosine_similarity([juego_filtrado_vector], [np.array([1 if genre in x else 0 for genre in genero_juego])])[0][0])
